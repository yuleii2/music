...
fun WorkbenchScreen(
    ...
    val tools = listOf(
        ...
        ToolDefinition("transpose", "移调与变调夹", "移动单个和弦或整段进行，并同步处理 slash bass。", "离线 · -11…+11", Icons.Rounded.SwapHoriz),
    ...
    LazyVerticalGrid(
        ...
        tools.firstOrNull { it.id == recentToolId }?.let { recent ->
        ...
        items(tools, key = { it.id }, contentType = { "tool" }) { tool ->
